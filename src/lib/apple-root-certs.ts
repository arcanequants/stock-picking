// Apple root CA certificates (DER, base64-embedded) used by the
// App Store Server Library's SignedDataVerifier to validate the x5c chain
// on signed transactions and App Store Server Notifications V2.
//
// Embedded (not read from disk) so the Vercel serverless bundle always
// ships them. Source: https://www.apple.com/certificateauthority/
//   - Apple Root CA - G3 (the chain that signs StoreKit/ASSN JWS)
//   - Apple Root CA - G2 (included for completeness)
// Both valid until 2039-04-30.

const APPLE_ROOT_CA_G3_B64 =
  "";

const APPLE_ROOT_CA_G2_B64 =
  "";

export const APPLE_ROOT_CERTS: Buffer[] = [
  Buffer.from(APPLE_ROOT_CA_G3_B64, "base64"),
  Buffer.from(APPLE_ROOT_CA_G2_B64, "base64"),
];
