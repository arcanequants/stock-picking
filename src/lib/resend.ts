...
export async function sendDigestApprovalEmail(
  ...
  const digestPreview = buildDigestHtml(events, "es", summary, weeklyDivs);
...
export async function sendDigestEmail(
  ...
  const { error } = await getResend().emails.send({
    ...
    subject,
    html: isSubscribed
      ? buildDigestHtml(events, locale, summary, weeklyDivs)
      : buildFreeDigestHtml(events, locale, summary, weeklyDivs),
  });
