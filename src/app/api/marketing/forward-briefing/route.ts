...
type Draft = { n: number; region: string; text: string };
type TableRow = { n: number; region: string; angle: string; worker?: string };
type Body = {
  date: string;
  summary?: string;
  causalChain?: string;
...
export async function POST(request: Request) {
  ...
  const fmtDate = new Date(body.date + "T00:00:00").toLocaleDateString("es-MX", {
    weekday: "long",
    day: "numeric",
    month: "long",
    year: "numeric",
  });
  const subject = `Briefing Vectorial Data — ${fmtDate} (${body.drafts.length} drafts X listos)`;
  ...
  try {
    ...
    return NextResponse.json({ success: true, id: result.data?.id, drafts: body.drafts.length });
  } catch (e) {
