/**
 * Backfill all existing picks as EAS attestations on Base L2.
...
async function backfill() {
  console.log(`Backfilling ${transactions.length} picks as EAS attestations...\n`);
  ...
  for (let i = 0; i < transactions.length; i++) {
    ...
    console.log(`[${i + 1}/${transactions.length}] ${tx.ticker} — $${tx.price} — ${tx.date}`);
    console.log(`  Chain hash: ${hash.slice(0, 16)}...`);

    if (tx.attestation_uid) {
      console.log(`  Already attested: ${tx.attestation_uid.slice(0, 16)}...`);
      previousHash = hash;
      continue;
    }
    ...
    try {
      ...
      // Rate limit: wait 2 seconds between attestations
