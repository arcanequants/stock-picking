...
## Stock Pick Workflow
When adding a new stock: fetch Yahoo `regularMarketOpen` (NOT `regularMarketPrice`) → research → add to stocks.ts → add transaction → generate WhatsApp message → `npm run mom:generate -- $TICKER` → update cycle → build → backfill → commit → push.
...
Plain-Spanish rewrites of `summary_short` / `summary_why` / `summary_risk` for the iOS pick detail screen. Generated by `scripts/generate-mom-shorts.ts` (LLM rewrite, gpt-4o), committed to repo, preferred by `/api/picks/research/[ticker]` over the compactor fallback.
- For a new pick: run `npm run mom:generate -- $TICKER` after adding to `stocks.ts` (script appends just that ticker)
- Re-run for an existing ticker after editing its source text: `npm run mom:generate -- $TICKER --force`
- Full re-backfill (rare): `npm run mom:generate -- --force`
